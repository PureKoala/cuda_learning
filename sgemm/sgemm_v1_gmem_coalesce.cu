...
/*
    V1 using global memory to perform SGEMM:
    -> Need to read a row of A and a col of B for each thread
    -> V0's each contiguous thread uses different row
    -> V1's each contiguous thread uses same row but different col
    -> just change the idx for x and y
