...
/*
    ...
    -> Each thread block computes a col of C of size TM * 1
...
__global__ void sgemm_kernel(
    ...
    for(int bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for(int dotIdx = 0; dotIdx < BK; dotIdx++) {
            ...
            for(int resIdx = 0; resIdx < TM; resIdx++) {
                threadRes[resIdx] += As[(threadRow * TM + resIdx) * BK + dotIdx] * tmpB;
            }
        }
    ...
    for(int resIdx = 0; resIdx < TM; resIdx++) {
        C_ptr[(threadRow * TM + resIdx) * N + threadCol] = threadRes[resIdx];
    }
