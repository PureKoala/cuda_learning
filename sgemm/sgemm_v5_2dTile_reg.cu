...
/*
    V5 impove BLOCK_SIZE and more Tile to perform SGEMM:
    ...
    -> Using Regfile to store A and B block read from SMEM
...
__global__ void sgemm_kernel(
    ...
    for(int bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for(int dotIdx = 0; dotIdx < BK; dotIdx++) {
            for(int i = 0; i < TM; i++) {
                A_reg[i] = As[(threadRow * TM + i) * BK + dotIdx];
            }
            for(int i = 0; i < TN; i++) {
